...
Considering the space of possible network configurations is highly irregular, there is likely a network configuration using convolution layers 
...
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth]{figures/regression/snap/cutoff-comp.png} %TODO: REPLACE!!!!
  \caption[Comparison of SNAP cutoff radii]{
  (a) Networks with different $n_{max}, l_{max}$ are trained on different cutoff radii.

  (c) Networks are trained with a fixed $r_{cut} = 12 \AA$ and a cutoff sphere $r_{del}$ outside of where all atoms are removed.
  (b) shows the percentage of atoms included in the cutoff sphere with radius $r_{del}$.
  All networks are trained on a train fraction of 80\%.
 }
  \label{fig:snap_hyperparameter}
\end{figure}

For $r_{del}=8$ the mean MAE over all networks trained on 80\% of the data was at $MAE = 0.734 kcal/mol$. 
...
The objective was to achieve similar performance with a feature descriptor that allowed for a mapping back from feature space to 3D space.
...
The first approach to regression was using LEFD features.
...
Similarly, a low density should indicate adding more density for this species will decreased the barrier. %TODO: So rum richtig??
